...
with st.sidebar:
    ...
    st.write("Columnas = estado actual, filas = siguiente estado (cada columna debe sumar 1)")

    normalize = st.checkbox("Normalizar columnas automáticamente", value=True)
    ...
    st.session_state.transition_df = st.data_editor(
    ...
    if st.button("Generar matriz aleatoria", use_container_width=True):
        seed_value_init = 2026 if usar_semilla else None
        rng_init = np.random.default_rng(seed_value_init)
        st.session_state.transition_df = _random_transition_df(n_samples=max(200, n_dias), rng=rng_init)
        st.rerun()
